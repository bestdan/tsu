...
describe('dartHookAnalysisCheck', () => {
    ...
    it('should exit with error when dart analyze finds issues', () => {
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith('❌ Push blocked: dart analyze found issues in the following file(s):');
        expect(consoleErrorSpy).toHaveBeenCalledWith('  lib/main.dart');
        expect(consoleErrorSpy).toHaveBeenCalledWith('Run `dart fix --apply` to fix some issues automatically.');
        expect(processExitSpy).toHaveBeenCalledWith(1);
        dartAnalyzeSpy.mockRestore();
    });
});
