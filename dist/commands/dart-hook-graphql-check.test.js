...
describe('dartHookGraphqlCheck', () => {
    ...
    it('should run codegen and exit with success if no changes created', () => {
        ...
        expect(execSyncMock).toHaveBeenCalledWith('melos run codegen:graphql', expect.any(Object));
        expect(execSyncMock).toHaveBeenCalledWith('melos run codegen:graphql:test', expect.any(Object));
        expect(processExitSpy).toHaveBeenCalledWith(0);
    });
    it('should exit with error if codegen creates changes', () => {
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith('   lib/graphql/query.fakes.dart');
        expect(processExitSpy).toHaveBeenCalledWith(1);
    });
    it('should exit with error if codegen command fails', () => {
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith('Error: Failed to run GraphQL code generation');
    ...
    it('should show verbose messages when verbose is true', () => {
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith('🧪 Checking for modified GraphQL files...');
        expect(consoleErrorSpy).toHaveBeenCalledWith('📝 Found modified GraphQL files: 2');
        expect(consoleErrorSpy).toHaveBeenCalledWith('  lib/graphql/query.graphql');
        expect(consoleErrorSpy).toHaveBeenCalledWith('  lib/graphql/mutation.graphql');
        expect(consoleErrorSpy).toHaveBeenCalledWith('🔧 Running GraphQL code generation...');
        expect(consoleErrorSpy).toHaveBeenCalledWith('✓ GraphQL fakes are up to date');
    });
});
//# sourceMappingURL=dart-hook-graphql-check.test.js.map