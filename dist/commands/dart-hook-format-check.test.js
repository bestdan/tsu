...
describe('dartHookFormatCheck', () => {
    ...
    it('should exit with error if formatting creates changes', () => {
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith('❌ Push blocked: Files were formatted. Please stage and commit these changes:');
    ...
    it('should exit with error if dart format command fails', () => {
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith('Error: Failed to run dart format');
        expect(consoleErrorSpy).toHaveBeenCalledWith('dart format failed');
        expect(processExitSpy).toHaveBeenCalledWith(1);
    });
});
//# sourceMappingURL=dart-hook-format-check.test.js.map