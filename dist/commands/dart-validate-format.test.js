...
describe('dartValidateFormat', () => {
    ...
    it('should exit with success if no staged Dart files to validate', () => {
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith('✓ No staged Dart files to validate');
    ...
    it('should validate packages when PACKAGE_INDEX exists', () => {
        ...
        expect(execSync).toHaveBeenCalledWith('dart format --set-exit-if-changed .', expect.objectContaining({ cwd: expect.stringContaining('packages/app') }));
        expect(execSync).toHaveBeenCalledWith('dart format --set-exit-if-changed .', expect.objectContaining({ cwd: expect.stringContaining('packages/core') }));
        expect(processExitSpy).toHaveBeenCalledWith(0);
    });
    it('should validate specific files when provided', () => {
        ...
        expect(execSync).toHaveBeenCalledWith(expect.stringContaining('dart format --set-exit-if-changed'), expect.anything());
        expect(processExitSpy).toHaveBeenCalledWith(0);
    });
    ...
    it('should filter out generated files', () => {
        ...
        const formatCall = execCalls.find((call) => call[0].toString().includes('dart format'));
        expect(formatCall).toBeDefined();
        expect(formatCall[0].toString()).not.toContain('user.g.dart');
        expect(formatCall[0].toString()).not.toContain('user.freezed.dart');
    });
});
//# sourceMappingURL=dart-validate-format.test.js.map