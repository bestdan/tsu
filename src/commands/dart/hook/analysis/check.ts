...
/**
 * Runs dart fix on Dart files and checks if analysis created changes.
...
export function dartHookAnalysisCheck(
  ...
  if (filesWithChanges.length > 0) {
    console.error('');
    console.error(
      '❌ Push blocked: Analysis fixes were applied. Please stage and commit these changes:'
    );
  ...
  logIfVerbose(verbose, '✓ All files pass dart fix analysis');
  process.exit(0);
}
